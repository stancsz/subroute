...
3. **控制面快速切模型：下拉菜单动态指定当前模型**：
   - 提供直观极简的网页控制台（访问生产环境 **`http://127.0.0.1:4000/m`** 或 **`/s`**）。
...
- [ ] 编写 `src/unified_llm_gateway/plugins/dynamic_router.py`：
  - 挂载 `/m` 与 `/s` 极简控制台页面（包含现代轻量下拉菜单与生效状态提示）。
...
- [ ] 验证本地打开 `http://127.0.0.1:4000/m` 下拉切换后，请求 `http://127.0.0.1:4000/v1` 的动态生效表现。
...
3. **控制面体验验收**：
   - 浏览器打开生产控制面 `http://127.0.0.1:4000/m` 呈现直观的下拉菜单。
